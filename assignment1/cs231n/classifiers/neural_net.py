...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        ## First layer: relu(linear_model)
        layer_1 = relu(np.matmul(X, W1) + b1)
        ## Second layer: scores = linear_model
        ...
        dOutput = output.copy()
        dOutput[range(N), y] -= 1
        dOutput /= N # Average gradients
        
        # Second layer
        ## Chain rule
        grads["W2"] = np.matmul(layer_1.T, dOutput)
        ## Sum rule
        grads["b2"] = np.sum(dOutput, axis = 0)
        
        # Relu Activation
        ## Get gradient for relu
        dlayer_1 = np.matmul(dOutput, W2.T)
        ## Backpropagate relu
        dlayer_1[layer_1 <= 0] = 0
        
        # First layer
        ## Chain rule
        grads["W1"] = np.matmul(X.T, dlayer_1)
        ## Sum rule
